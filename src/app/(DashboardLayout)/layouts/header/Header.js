...
const Header = ({ showMobmenu, children }) => {
  ...
  const handleButton = (buttonAc) => {
    setActiveButton(buttonAc);
      ...
      <Collapse navbar isOpen={isOpen}>
        <Nav className="me-auto w-100 d-flex align-items-center justify-content-between" navbar>
          ...
          <span className="d-flex align-items-center gap-3">
            {!currentUser ? (
              <span className="d-flex align-items-center gap-3">
                ...
                <UncontrolledDropdown nav onClick={() => handelActive('features')}>
                  ...
                  <DropdownMenu end>
                    <Link href="/pages/bank-soal" onClick={() => handelActive('features')}>
                      <DropdownItem>
                        Bank Soal
                      </DropdownItem>
                    </Link>
                    <DropdownItem divider />
                    <Link href="/pages/try-out" onClick={() => handelActive('features')}>
                      <DropdownItem>
                        Try Out
                      </DropdownItem>
                    </Link>
                    <DropdownItem divider />
                    <Link href='/pages/rekomendasi-belajar'>
            ...
            <div className="d-flex gap-2 lg-block">
              {!currentUser ? (
                <NavItem className="bg-transparent border-2 border-primer rounded-3 p-0" style={{ width: '200px' }}>
                  <Button
                    className={`border w-50 h-100 m-0 border-0 ${activeButton === 'login' ? 'bg-primer text-white' : 'bg-transparent text-black'}`}
                    onClick={() => handleButton('login')}
                  >
                    Login
                  </Button>
                  <Button
                    className={`border w-50 h-100 m-0 border-0 ${activeButton === 'register' ? 'bg-primer text-white' : 'bg-transparent text-black'}`}
                ...
                <NavItem className="bg-graybold rounded-5 p-0" style={{ width: '115px' }}>
                  <UncontrolledDropdown>
                    <DropdownToggle className='bg-transparent border-0 w-100'>
                      <Button className="border w-100 h-100 m-0 border-0 p-0 bg-transparent justify-content-between align-items-center rounded-5 d-flex">
